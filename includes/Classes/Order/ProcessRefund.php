...
namespace Merchant_E_Solutions\Includes\Classes\Order;
		...
		public function run( $order_id, $amount = null ) {
			...
			if ( '000' === $tran->getResponseField( 'error_code' ) ) {
				...
				if ( $order->get_total() === $amount ) {
					...
					$message = sprintf( esc_html__( 'The Order has been completely refunded a total of %1$s.', 'merchant-e-solutions' ), wc_price( $amount ) );
					...
					/* translators: The refund amount */
					$message = sprintf( esc_html__( 'The Order has been refunded a total of %1$s.', 'merchant-e-solutions' ), wc_price( $amount ) );
					$order->add_order_note( $message );
				}
